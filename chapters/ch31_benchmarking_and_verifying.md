...
short_title: "ch31 Benchmarking and Verifying"
---
...
```{include} _generated/ch31-generators.md
```
...
**Distributional.** For anything sampled, "the same tokens" is the wrong standard. {ref}`ch17`'s
...
1. **State the workload.** Prompt and output length *distributions*, not means. {ref}`ch22`'s table
...
- **Verification suites are expensive to build** and are the first thing cut. The distributional
  tests in {ref}`ch17` took longer to get right than the speculation implementation they check.
...
{ref}`ch32` puts the whole journey in one table, accounts for which changes paid for their
...
{ref}`ch17`'s list contain the proofs that make a distributional test meaningful — without them you
