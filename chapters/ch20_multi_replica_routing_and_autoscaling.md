...
short_title: "ch20 Multi-Replica"
---
...
```{include} _generated/ch20-routing.md
```
...
```{include} _generated/ch20-cold-start.md
```
...
3. **Quantisation ({ref}`ch15`) is an availability feature.** The INT4 rows are roughly a quarter
...
- **Per-replica metrics stop being enough.** The interesting quantities are now fleet-wide —
  ...
  inside a single engine. {ref}`ch27` picks this up.
...
This chapter routed by prefix and treated every request as equally entitled to service. {ref}`ch21`
