...
short_title: "ch18 Constrained Decoding"
---
...
Outlines and XGrammar are largely engineering around that problem. Chapter 18 gets to skip it, and
...
```{include} _generated/ch18-validity.md
```
...
```{include} _generated/ch18-mask-cost.md
```
...
- **Interaction with {ref}`ch17`** is awkward: a drafter that does not know the grammar proposes
...
{ref}`ch19` needs more than one GPU and is written against hardware this book's default tier does
not have.
