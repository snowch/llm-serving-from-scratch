...
short_title: "ch14 Quantisation"
---

(ch14)=
# ch14 · Quantisation for Serving
...
Weights are a fixed cost. The KV cache is per concurrent request, and {ref}`ch12` showed it is what
...
```{include} _generated/ch14-quantisation.md
```
...
```{include} _generated/ch14-kv-quantisation.md
```

Quantising the cache is nearly free in quality and halves or quarters the per-token footprint.
Combined with {ref}`ch12`'s arithmetic — where an 8B model at 32k context holds ten concurrent
...
```{include} _generated/ch14-outliers.md
```
...
Quantisation made each step cheaper. {ref}`ch15` attacks something else entirely: the fact that
