...
short_title: "ch29 Cost and Capacity"
---
...
| **Prerequisites** | [ch03](#ch03), [ch07](#ch07), [ch20](#ch20) |
...
```{include} _generated/ch29-cost.md
```
...
The corollary is that most of {ref}`ch20` is a cost chapter in disguise. Routing, autoscaling and
...
changing. {ref}`ch23`'s retrieval workload is enormously prompt-heavy and is therefore cheaper per
billed token than {ref}`ch22`'s chat, on identical hardware, while being harder to serve. The two
statements are not in tension: one is about machine time, the other about what you count.

## Build versus buy

```{include} _generated/ch29-break-even.md
```
...
{ref}`ch28`, and the capacity headroom you pay for and do not use.
...
reaches one is not the batch size that meets the other ({ref}`ch22`).

So sizing has three steps, in this order:

1. **Pick the batch size that meets the SLO**, from {ref}`ch22`'s table for your workload.
...
3. **Divide demand by that, then by your real utilisation**, and add headroom for the cold start in
   {ref}`ch20` — because an autoscaler is late by exactly that much.
...
- **Optimising cost and optimising latency pull apart.** Higher utilisation is cheaper and queues
  more; {ref}`ch22`'s chat tuning is deliberately more expensive per token than {ref}`ch25`'s batch
...
- **Spot and preemptible capacity change the model** in a way this chapter does not cover: they cut
  the hourly rate substantially and add a failure mode that {ref}`ch28`'s draining is the only
...
Every number in this chapter came from a throughput figure, and {ref}`ch31` is about where that figure
