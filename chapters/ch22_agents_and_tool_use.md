...
{ref}`ch09`, and {ref}`ch20`'s framing table shows it: agent traffic has the longest prompts in the
...
```{include} _generated/ch22-chains.md
```
...
```{include} _generated/ch22-cancellation.md
```
...
- **The chain is invisible to the engine.** Everything in this chapter about chain latency has to be
  ...
  trace id threaded through the API ({ref}`ch24`) and honoured by the metrics ({ref}`ch25`).
- **Optimising the median trades away the tail**, and if the same fleet also serves interactive
  ...
  at least for {ref}`ch19`'s per-tenant policy.
...
- **Retries multiply everything.** An agent that retries a failed step re-submits a prompt that is
  ...
  agent framework is the noisy neighbour of {ref}`ch19`.
...
{ref}`ch23` closes Part VI with the two extremes of the latency axis: code completion, where a
