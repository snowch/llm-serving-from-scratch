...
short_title: "ch26 The API Surface"
---
...
| **Prerequisites** | [ch04](#ch04), [ch09](#ch09), [ch24](#ch24) |
...
each is invisible to everything in {ref}`ch27`'s dashboard.
...
```{include} _generated/ch26-templates.md
```
...
The same applies to cancellation. {ref}`ch24` built abort into the engine and made the point there:
...
model is built on ({ref}`ch21`, {ref}`ch29`), so a systematic error here propagates into decisions
that look unrelated.
...
to itself, where it is less visible and harder to shed. {ref}`ch28` is the answer.

**Request size limits.** An unbounded prompt is an unbounded prefill, which is an unbounded stall
for everyone else ({ref}`ch23`). The limit belongs at the edge, in tokens rather than bytes, and it
...
- **Cancellation plumbing crosses every layer** — client disconnect, HTTP handler, engine abort — and
  is only correct if all three agree. {ref}`ch24` lists the races.
...
the metrics. {ref}`ch27` builds the signals that would have caught these, and measures which one
