...
| **Prerequisites** | [ch02](#ch02), [ch08](#ch08), [ch25](#ch25) |
...
Not when the CPU is busy — {ref}`ch25` covered why that number does not track the constraint. The two
signals that do:
...
**A rejection must reach the caller.** The implementation reuses {ref}`ch22`'s cancellation path for
exactly this reason:
...
```{include} _generated/ch26-drain.md
```
...
**NaN or inf in the output.** {ref}`ch06` and {ref}`ch16` both found the same cause: masking with
...
nothing in {ref}`ch25`'s dashboard moves. A quantisation setting, a sampling default, a chat template
({ref}`ch24`) — all change what the model says while leaving every latency and throughput signal
untouched. The defence is not monitoring; it is the equivalence and distribution tests from
{ref}`ch15`, run in CI, on every change that touches the numerical path.
...
4. **Cache hit rate dropped** → somebody changed a prompt or a template ({ref}`ch24`), or routing
   changed ({ref}`ch18`). Almost never the cache.
5. **One tenant complaining, aggregate healthy** → {ref}`ch19`. The aggregate is hiding them.
...
- **Shedding rejects real customers**, and the decision of *whose* request to drop is a policy nobody
  wants to own. Per-tenant weights ({ref}`ch19`) at least make it explicit rather than arbitrary.
- **Thresholds need tuning**, and they are workload-dependent: a queue depth that is healthy for
  {ref}`ch23`'s batch traffic is an emergency for {ref}`ch20`'s chat.
...
{ref}`ch27` asks what all of this costs. The answer depends far less on the hardware than anyone
...
why the engine here charges an *estimate* ({ref}`ch19`) rather than a measurement.
