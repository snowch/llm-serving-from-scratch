...
short_title: "ch22 Chat and Assistants"
---
...
```{include} _generated/ch22-workloads.md
```
...
```{include} _generated/ch22-turns.md
```
...
{ref}`ch20`'s session affinity worth more here than anywhere else — routing a returning user to a
...
```{include} _generated/ch22-batch.md
```
...
from the one {ref}`ch25`'s offline workload gets from the same table.
...
- **Streaming is the API contract**, not an optimisation. {ref}`ch26` covers the mechanics; what
...
- **Session affinity is worth more than load balance** ({ref}`ch20`), for exactly the reuse reason
  above.
...
- **Tuning for ITL costs throughput**, and the table above prices it. A chat deployment deliberately
  runs at lower utilisation than a batch one, and that shows up in {ref}`ch29`'s cost model as a
...
- **Session affinity is a scheduling constraint**, and it fights every other routing goal. The
  imbalance guard from {ref}`ch20` is doing real work here.
...
- Pick the smallest batch that saturates throughput. {ref}`ch25` picks the largest that fits, from
...
The framing table's worst row is retrieval, and it is worst by two orders of magnitude. {ref}`ch23`
