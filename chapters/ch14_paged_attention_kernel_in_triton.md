...
short_title: "ch14 Paged Attention Kernel"
---
...
| **Prerequisites** | [ch08](#ch08), [ch13](#ch13) |
...
{ref}`ch13`'s reference on every shape that matters, and a precise account of the translation. The
...
```{include} _generated/ch14-gather-cost.md
```
...
{ref}`ch13` already contains the key. Online softmax processes keys one block at a time, maintaining
...
plus {ref}`ch13`'s streaming softmax.
...
- {ref}`ch08`'s gather roughly halves the decode ceiling, and the cost is pure data movement rather
  than arithmetic.
- **Online softmax is what makes a paged kernel possible.** Because {ref}`ch13`'s algorithm never
...
{ref}`ch15` goes after the same bottleneck from the other direction. This chapter halved the bytes
