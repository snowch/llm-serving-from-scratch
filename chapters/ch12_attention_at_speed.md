...
| | |
|---|---|
| **Tier** | Tier 1 — CPU runnable; the kernel work in [ch13](#ch13) needs a GPU |
...
```{include} _generated/ch12-score-matrix.md
```
...
```{include} _generated/ch12-gqa.md
```
...
```{include} _generated/ch12-footprint.md
```
...
themselves smaller: {ref}`ch14` stores weights and cache in fewer bits, and is the first chapter
where an optimisation makes output measurably *worse* as well as faster.

{ref}`ch13` writes the paged-attention kernel that {ref}`ch08` has been waiting for. It needs a
