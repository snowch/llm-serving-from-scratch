...
```{include} _generated/ch19-collectives.md
```
...
({ref}`ch13`) the binding constraint is the *key/value* head count, not the query head count. A model
...
```{include} _generated/ch19-bubbles.md
```
...
out of scope; {ref}`ch32` lists it among the deliberate omissions.
...
- **Per-device efficiency always drops.** Two devices never give twice the throughput of one, and the
  ...
  do to go faster — for that, {ref}`ch20`'s replicas are strictly better, because independent
...
{ref}`ch20` takes exactly that last point seriously. If the model fits on one device, the way to
