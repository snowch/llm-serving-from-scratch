...
short_title: "ch23 RAG and Long Context"
---
...
| **Prerequisites** | [ch09](#ch09), [ch10](#ch10), [ch15](#ch15) |
| **Scorecard** | The worst row in Part VI's table, and why none of the obvious fixes work. |
:::

## The problem

Go back to {ref}`ch22`'s framing table and compare two rows. Retrieval and agent traffic have prompts
...
```{include} _generated/ch23-interference.md
```
...
({ref}`ch28`). A scheduler is not a capacity plan.
...
**Quantise the KV cache** ({ref}`ch15`). Retrieval is where the KV footprint binds first, because
every request holds blocks proportional to its very long prompt. Halving the bytes per token nearly
doubles concurrency, and {ref}`ch15`'s measurement is that the quality cost of INT8 KV is small.
...
- **KV memory is the binding constraint**, not compute. Concurrency is set by how many long prompts
  fit, which is why {ref}`ch15`'s quantisation matters more here than in any other workload.
...
Retrieval was the workload where reuse was worth least. {ref}`ch24` is the other extreme: agent
...
{ref}`ch15`'s list apply directly, and the cache-quantisation results are the relevant ones here
