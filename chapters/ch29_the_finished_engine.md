...
```{include} _generated/ch29-journey.md
```
...
| Paged attention ({ref}`ch08`) | Pays for itself through concurrency, and everything after it depends on the block allocator. Cancellation ({ref}`ch22`) and quotas ({ref}`ch19`) are cheap only because it exists. |
| Cache-aware routing ({ref}`ch18`) | A policy change, no new machinery, and it turned zero goodput into real goodput. Best value in Part V. |
| Fair queueing ({ref}`ch19`) | One overridden method. Does not create capacity; decides who waits, which was the entire problem. |
| Load shedding ({ref}`ch26`) | Small, and the difference between degrading and failing. |
...
| Quantisation ({ref}`ch14`) | Worth it for memory and for cold starts ({ref}`ch18`). Sold as a speedup, delivered as capacity. |
| Speculative decoding ({ref}`ch15`) | Genuinely useful at low batch size, and the correctness argument is subtle enough that most of the value of the chapter is knowing what to check. |
...
| Constrained decoding ({ref}`ch16`) | Cheap, and buys a guarantee weaker than the one people assume they are buying. |
...
- **Multi-node serving.** Everything here fits on one machine. Crossing a node boundary changes the
  interconnect by an order of magnitude, which changes which of {ref}`ch11` and {ref}`ch17` are
...
- **Mixture-of-experts routing at scale.** Expert parallelism has a scheduling problem this book does
  ...
  way no static plan fixes. {ref}`ch17` sketches it; serving a real MoE needs more.
- **Custom CUDA beyond {ref}`ch13`'s Triton kernel.** The kernel chapter shows the shape of the work
...
- **A real model.** The reference model is built from seeded random weights so that every
  ...
  output in this book is meaningful text, and quality claims ({ref}`ch14`, {ref}`ch15`,
  {ref}`ch19`) are made against a small model trained on a synthetic corpus rather than against a
  production one.
- **A cross-engine comparison.** {ref}`ch28` builds the methodology and deliberately does not report
...
- **Security and abuse.** Rate limits appear as quotas in {ref}`ch19` and nothing else does. Prompt
...
**SGLang** is where to look for {ref}`ch09` and {ref}`ch16` done seriously. RadixAttention is prefix
caching with a radix tree instead of a flat hash map, which handles branching conversations that this
book's cache handles poorly. Its structured-output path is {ref}`ch16` with the token-lifting problem
actually solved, and its router is {ref}`ch18`.
...
{ref}`ch07`, {ref}`ch10` and {ref}`ch25`, and go look at your queue depth. If your throughput is bad,
you have a memory problem: {ref}`ch03`, {ref}`ch08` and {ref}`ch14`, and go compute your KV bytes per
token. If your costs are bad, {ref}`ch27` and go find your utilisation — it is almost certainly lower
than you think, and that is almost certainly the whole answer.

And if you are about to publish a performance comparison, {ref}`ch28` first.
