...
short_title: "ch25 Observability"
---
...
- **Prefix hit rate** ({ref}`ch09`) says *whether the workload is what you think it is* — and, per
  {ref}`ch24`, whether somebody changed the chat template.
...
```{include} _generated/ch25-overhead.md
```
...
```{include} _generated/ch25-signals.md
```
...
**tenant** ({ref}`ch19`) and **chain or session id** ({ref}`ch22`), because without the second the
...
- **Per-tenant dimensions multiply cardinality.** A histogram per tenant per endpoint is how a
  ...
  in traces — means the fairness problem of {ref}`ch19` is invisible in the metrics.
...
{ref}`ch25`'s signals say when the engine is in trouble. {ref}`ch26` decides what it should *do* about
